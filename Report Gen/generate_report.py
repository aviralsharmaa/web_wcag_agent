...
matching the HDFC SKY audit report format with annotated screenshots.
"""
...
def load_checklist_data(base_dir):
    ...
    screen_checklist_results = defaultdict(lambda: defaultdict(str))  # screen_tag -> {checklist_name: pass/fail}
...
def build_executive_dashboard(wb, app_name, unique_screens, all_issues, wcag_failures, checklists, run_metadata):
    ...
    cannot_verify = 0  # We don't have this data, estimate
...
def main():
    if len(sys.argv) < 2:
        # Default path
        base_dir = "/Users/aviral/Projects/android-test/projects/Archive/wcag_output/com-icicidirect-ilearn__v1-0-29__20260311_162354"
    else:
        base_dir = sys.argv[1]
    ...
    checklists, all_screens, all_issues, screen_issues, screen_passes = load_checklist_data(base_dir)
    ...
    # Deduplicate screens
    unique_screens = deduplicate_screens(all_screens, screen_issues, base_dir)
    ...
    if "ilearn" in dir_name.lower():
        app_name = "ICICI Direct iLearn"
    elif dir_name.startswith("com-"):
        app_name = dir_name.split("__")[0].replace("-", ".")
    ...
    output_file = os.path.join(output_dir, f"{app_name.replace(' ', '_')}_WCAG_Audit_Report.xlsx")
    wb.save(output_file)
